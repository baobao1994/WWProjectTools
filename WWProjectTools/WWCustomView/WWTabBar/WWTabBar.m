...
// selctButton
@property (weak , nonatomic) WWButton *selButton;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [[WWTabBarConfig shared] addObserver:self forKeyPath:@"textColor" options:NSKeyValueObservingOptionNew context:nil];
        [[WWTabBarConfig shared] addObserver:self forKeyPath:@"selectedTextColor" options:NSKeyValueObservingOptionNew context:nil];
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 0.5)];
        lineView.backgroundColor = [WWTabBarConfig shared].shadowBackgroundColor;
...
- (void)setItems:(NSArray<UITabBarItem *> *)items {
    ...
    for (int i=0; i<items.count; i++) {
        ...
        if (-1 != self.centerPlace && i == self.centerPlace) {
            self.centerBtn = [WWCenterButton buttonWithType:UIButtonTypeCustom];
            ...
            btn = [WWButton buttonWithType:UIButtonTypeCustom];
            //Add Observer
            ...
            [item addObserver:self forKeyPath:@"badgeColor"
                      options:NSKeyValueObservingOptionNew
                      context:(__bridge void * _Nullable)(btn)];
            [self.btnArr addObject:(WWButton *)btn];
            [btn addTarget:self action:@selector(controlBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        }
        //Set image
        [btn setImage:[item.image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
        [btn setImage:[item.selectedImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateSelected];
...
- (void)layoutSubviews {
    ...
    NSInteger mid = ({
        NSInteger mid = [WWTabBarConfig shared].centerBtnIndex;
    ...
    for (int i=0; i<count; i++) {
        if (i == mid && self.centerBtn!= nil) {
            ...
            ? CGRectMake(rect.origin.x,
                         -[WWTabBarConfig shared].bulgeHeight-h ,
                         rect.size.width,
                         rect.size.height+h)
            : rect;
        } else {
 ...
 */
- (void)controlBtnClick:(WWButton *)button {
 ...
 *  Center button click
 */
- (void)centerBtnClick:(WWCenterButton *)button {
...
- (void)setSelectButtoIndex:(NSUInteger)index {
    if (self.centerBtn && index == self.centerBtn.tag) {
        self.selButton = (WWButton *)self.centerBtn;
    } else {
        for (WWButton *loop in self.btnArr) {
            if (loop.tag == index) {
                self.selButton = loop;
                break;
            }
 ...
 */
- (void)setSelButton:(WWButton *)selButton {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"badgeValue"] || [keyPath isEqualToString:@"badgeColor"]) {
        WWButton *btn = (__bridge WWButton *)(context);
        btn.item = (UITabBarItem*)object;
    } else if ([object isEqual:[WWTabBarConfig shared]]) {
...
- (void)dealloc {
    ...
    [[WWTabBarConfig shared]removeObserver:self forKeyPath:@"textColor" context:nil];
    [[WWTabBarConfig shared]removeObserver:self forKeyPath:@"selectedTextColor" context:nil];
}
...
#warning 这里进行配置tabbar的名字
    WWTabBar *tabBar = [self.controller valueForKeyPath:@"wwtabbar"];
    if (CGRectContainsPoint([tabBar convertRect:tabBar.centerBtn.frame toView:self], point)) {
        return tabBar.centerBtn;
    }
    for (WWButton *loop in tabBar.btnArr) {
